...
def load_data(
  *,
  ...
  loader = DataLoader(
      ...
      shuffle=not deterministic, num_workers=1, drop_last=True
  )
  while True:
      yield from loader
...
def load_data_celeba(
  *,
  ...
  loader = DataLoader(
      dataset, batch_size=batch_size,
      shuffle=not deterministic, num_workers=5, drop_last=True
  )
  while True:
      yield from loader
...
class CelebADataset(Dataset):
  def __init__(
      ...
      self.transform = transforms.Compose([
          ...
          transforms.RandomResizedCrop(image_size, (0.95, 1.0)) if random_crop else transforms.Lambda(lambda x: x),
          transforms.ToTensor(),
          transforms.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5]) if normalize else transforms.Lambda(lambda x: x),
      ])
